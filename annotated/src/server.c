               ...
               :	}
               :/*
               :        if (getnameinfo(remotehost->ai_addr, remotehost->ai_addrlen, hbuf, sizeof(hbuf), NULL,
               :	    0, NI_NUMERICHOST) == 0)
               :	    printf("host=%s\n", hbuf);
               :	else
               :	    printf("%s\n", strerror(errno));
               :
               :	printf("canonname: %s\n",remotehost->ai_canonname);
               :	printf("ai_next:  %08X\n",(unsigned int) remotehost->ai_next);
               :*/									    
               :
               ...
               :	alarm(0);
               :/*
               :	addrlen = sizeof(remotehostserverip);
               :	if (getsockname(g.serversocket, remotehost->ai_addr, &addrlen) < 0) {
               :	    ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_CRITICAL, "getsockname error: %s", strerror(errno));
               :    	    close(g.serversocket);
               :    	    return T_ERROR;
               :	}
               :*/
               ...
               :    
               :    g.lastservercontact = g.currenttime;		    
               :    
               :    return 0;
               :
               :}
               :
   698  0.8001 :void server_read(void) { /* server_read total:   2203  2.5251 */
               :char tempstr[TEXTLENGTH_STD];
               :char tempbuff[TEXTLENGTH_MAX];
               :char server_input_line[TEXTLENGTH_MAX];
               :
  1119  1.2826 :    context();
               :
    31  0.0355 :    if ((g.serverstatus == SERVER_STATUS_TRYING) && FD_ISSET(g.serversocket, &g.writeset)) {
               ...
               :	    g.serverstatus = SERVER_STATUS_CONNECTED;
               :            FD_CLR(g.serversocket, &g.writeset);
               :
               :	    if (fcntl(g.serversocket, F_SETFL, O_NONBLOCK) < 0)
               :		ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_WARN, "couldn't set nonblocking error: %s", strerror(errno));
               ...
               :	    	    ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_INFO, "ourip = %d.%d.%d.%d", 
               ...
               :	    server_write(tempstr, SERVER_SEND_NOW);
               :	    if (g.user_modes && strlen(g.user_modes)) {
               :        	snprintf(tempstr, TEXTLENGTH_STD - 1, "MODE %s %s", g.user_nick, g.user_modes);
               :	        server_write(tempstr, SERVER_SEND_NOW);
               :	    }
               :		     	    
               :	}
               :    }
               :    
    63  0.0722 :    if (g.serverstatus == SERVER_STATUS_CONNECTED && FD_ISSET(g.serversocket, &g.readset)) {
               :	int length=0, i=0, j=0;
               :	
               :	update_timeval(g.lastservercontact, g.currenttime);
               :	bzero(tempbuff, TEXTLENGTH_MAX);
               :	length = read(g.serversocket, &tempbuff, TEXTLENGTH_MAX);
               :
               :        if (length < 1) {
               ...
               :            i = 0;
               :            j = g.linecut;
               :            while ((i < TEXTLENGTH_MAX) && (tempbuff[i] != '\0')) {
               :                while ((i < TEXTLENGTH_MAX) && (tempbuff[i] != '\n') && (tempbuff[i] != '\0')) {
               :                    server_input_line[j] = tempbuff[i];
               :                    i++;
               :                    j++;
               :                }
               ...
               :		    server_parseline(stripnonprintable(server_input_line));
               :		    ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_DEBUG_EXTRA, ">RCV>: %s", server_input_line);
               :            	    g.linecut = 0;
               :		}
               :		j = 0;
               :		i++;
   ...
   292  0.3347 :}
               ...
               :	snprintf(nickbuff, TEXTLENGTH_STD - 1, "%s%ld", g.user_nick, g.currenttime.tv_sec & 255);
               :	strncpy(g.user_nick, nickbuff, TEXTLENGTH_STD - 1);
               :	snprintf(tempstr, TEXTLENGTH_MAX - 2, "NICK %s", g.user_nick);
               :        server_write(tempstr, SERVER_SEND_NOW);
               :	snprintf(tempstr, TEXTLENGTH_MAX - 2, "USER %s * * :%s", g.user_login, g.user_realname);
               :	server_write(tempstr, SERVER_SEND_NOW);
               ...
               :}
/* 
 * Total samples for file : "src/server.c"
 * 
 *   2203  2.5251
 */
