...
int retry=0;
    ...
    global_func[3] = (Function) context_update;
    ...
    global_func[12] = (Function) stripnonprintable;
	...
	if (p->hooks & MODULE_HOOK_EVERY1SEC) {
	    p->mod_every1sec = dlsym(p->handle, "mod_every1sec");
	    if ((error = dlerror()) != NULL)
		ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_CRITICAL, "dlsym/every1sec(%s): %s", p->name, error);
	}
	...
	if (p->hooks & MODULE_HOOK_DUMPSTATUS) {
	    p->mod_dumpstatus = dlsym(p->handle, "mod_dumpstatus");
	    if ((error = dlerror()) != NULL)
		ioutput(OUTPUT_DEST_LOG | OUTPUT_TYPE_CRITICAL, "dlsym/dumpstatus(%s): %s", p->name, error);
	}
	p = p->next;
    }
...
struct module_list_t *p = NULL;
    ...
    while (p) {
        if (p->hooks & MODULE_HOOK_SERVERQUIT) 
	    ((int (*)(struct global_t *)) p->mod_serverquit) (&g);
        p = p->next;
    }
    return;
}
...
struct module_list_t *p = NULL;
    ...
    while (p) {
        if (p->hooks & MODULE_HOOK_EVERY1SEC)
	    (( int (*) (struct global_t *)) p->mod_every1sec) (&g);
	p = p->next;
    }
    return;
}
...
struct module_list_t *p = NULL;
    ...
    while (p) {
        if (p->hooks & MODULE_HOOK_EVERY20SEC)
	    (( int (*) (struct global_t *)) p->mod_every20sec) (&g); 
	p = p->next;
    }
    return;
}
...
struct module_list_t *p = NULL;
    ...
    while (p) {
        if (p->hooks & MODULE_HOOK_DUMPSTATUS) 
	    (( int (*) (struct global_t *)) p->mod_dumpstatus) (&g);
	p = p->next;
    }
    return;
}
